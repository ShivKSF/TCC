...
if($valor == $cp9){
	...
	$pdo->query("UPDATE $pagina SET saida = '$saida', usuario_baixa = '$id_usuario', status = 'Paga', juros = '$valor_juros', multa = '$valor_multa', desconto = '$valor_desconto', subtotal = '$subtotal', data_baixa = curDate() WHERE id = '$id'");
	...
	if(@$dias_frequencia > 0){
		$pdo->query("INSERT INTO $pagina SET descricao = '$cp1', saida = '$cp3', documento = '$cp4', plano_conta = '$cp5', data_emissao = curDate(), vencimento = '$nova_data_vencimento', frequencia = '$cp8', valor = '$cp9', id_usuario_lancamento = '$id_usuario', status = 'Pendente', data_recor = '$data_recor'");
...
//LANÇAR NAS MOVIMENTAÇÕES
	$pdo->query("INSERT INTO movimentacoes SET tipo = 'Saída', movimento = 'Conta à Pagar', descricao = '$descricao_conta', valor = '$subtotal', usuario = '$id_usuario', data = curDate(), lancamento = '$saida', plano_conta = '$cp5', documento = '$cp4', caixa_periodo = '$caixa_aberto', id_mov = '$id_compra'");
