...
if($alterou_data == 'Sim'){
	...
	$query = $pdo->query("SELECT * FROM $pagina WHERE (vencimento >= '$dataInicial' and vencimento <= '$dataFinal') and status LIKE '$status' and id_venda != '-1' order by id desc ");
	}
}else if($status != '%%' and $alterou_data == ''){
	$query = $pdo->query("SELECT * FROM $pagina WHERE status LIKE '$status' and id_venda != '-1'  order by id desc ");
}

else if($vencidas == 'Vencidas'){
	$query = $pdo->query("SELECT * FROM $pagina WHERE vencimento < curDate() and status = 'Pendente' and id_venda != '-1' order by id desc ");
}
	...
	$query = $pdo->query("SELECT * FROM $pagina WHERE vencimento = '$data_amanha' and status = 'Pendente' and id_venda != '-1' order by id desc ");
}
...
for($i=0; $i < @count($res); $i++){
	...
	if($ext == 'pdf'){ 
		...
		$query1 = $pdo->query("SELECT * FROM pessoas WHERE id = '$cp2' AND aluno = 1 ");
		$res1 = $query1->fetchAll(PDO::FETCH_ASSOC);
		if(@count($res1) > 0){
